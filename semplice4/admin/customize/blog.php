		...
		public function get_css($is_admin) {
			...
			if(is_array($options)) {
				...
				if(isset($options['blog_alignment'])) {
					$css .= '.post .row { justify-content: ' . $options['blog_alignment'] . '; }';
				}
				// bg color
				if(isset($options['blog_bg_color'])) {
					if($is_admin) {
						...
						$css .= '#content-holder, #content-holder .posts { background-color: ' . $options['blog_bg_color'] . '; }';
					}
				}
				// display content
				...
				if(isset($options['blog_title_alignment'])) {
					$css .= '.post .post-heading h2 a, .posts .post-heading h2 { text-align: ' . $options['blog_title_alignment'] . '; }';
				}
				// blog title color
				...
				if(!$is_admin) {
					...
					if(isset($options['comment_title_font_family'])) {
						$css .= '.comments-title { ' . semplice_get_font_family($options['comment_title_font_family']) . ' }';
					}
					// comment author font
