		...
		public function output_editor($values, $id) {
			...
			if(is_array($images) && !empty($images)) {
				...
				foreach ($images as $image) {
					...
					$img['caption'] = $image_post->post_excerpt;
					...
					// image html
					if($target == 'lightbox') {
						$image = '<a class="semplice-lightbox gallerygrid-image mouseover-' . $mouseover . '"><img class="lightbox-item" src="' . $img[0] . '" width="' . $img[1] . '" height="' . $img[2] . '" caption="' . $img['caption'] . '">' . $mouseover_html . '</a>';
					} else {
