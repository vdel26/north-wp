	...
	class sm_portfoliogrid {
		...
		public function output_editor($values, $id) {
			...
			if($title_position != 'below') {
				$title_css = 'margin-left: ' . ($hor_gutter / 2) . 'px; margin-right: ' . ($hor_gutter / 2) . 'px;';
				// add margin bottom
				if(strpos($title_position, 'bottom') !== false) {
					$title_css .= 'margin-bottom: ' . $ver_gutter . 'px;';
				}
			...
			if(!empty($projects)) {
				...
				// add to css
				$add_to_css .= '#content-holder #' . $id . ' .thumb .post-title { ' . $title_css . ' } #' . $id . ' .thumb .post-title a { color: ' . $title_color . '; font-size: ' . $title_fontsize . '; text-transform: ' . $title_text_transform . '; } #' . $id . ' .thumb .post-title a span { color: ' . $category_color . '; font-size: ' . $category_fontsize . '; text-transform: ' . $category_text_transform . '; padding-top: ' . $category_padding_top . '; }';
