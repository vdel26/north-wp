...
function semplice_save_post($post_id) {
	...
	// save wp changes to semplice revisions to recognize changes are made within wordpress or project / pages overview in semplice
	global $wpdb;
	$rev_table_name = $wpdb->prefix . 'semplice_revisions';
...
function semplice_admin_styles_and_scripts($hook) {
	...
	if ($hook == $semplice_admin_page && $ref != 'instagram-auth') {

		// google webfont
		wp_enqueue_style('google_webfonts', 'https://fonts.googleapis.com/css?family=Open+Sans:300,300italic,400,400italic,600,600italic,700,700italic|Source+Code+Pro:400|Lato:400,400italic,700,700italic');
		...
		// atts
		require get_template_directory() . '/admin/atts.php';
		...
		wp_localize_script('semplice-admin', 'semplice', array(
			...
			'semplice_api_url'	=> home_url() . '/wp-json/semplice/v1/admin',
			'editor_api_url'	=> home_url() . '/wp-json/semplice/v1/editor',
			...
			'customize'			=> array(
				...
				'usedFonts'		=> json_decode(get_option('semplice_used_fonts')),
...
function wp_admin_head($hook) {
	...
	if($screen->id == 'toplevel_page_semplice-admin') {
		...
		// placeholders
		$head_placeholders = array('semplice-post-css', 'semplice-options-visibility', 'semplice-navigations', 'semplice-navigations-hover-css', 'semplice-navigations-hamburger', 'semplice-thumbhover-css', 'semplice-projectpanel-css', 'motion-duplicate-css', 'semplice-blog-css', 'semplice-cover-slider');
	...
	<style type="text/css">
		...
		#_is_semplice {	background: #ffd300; position: relative; }
		#_is_semplice h2 { border-bottom: 1px solid #e5bd00; }
		...
		.instagram-auth { width: 100%; height: 100%; position: fixed; background: #f5f5f5; top: 0; left: 0; bottom: 0; z-index: 500000; display: flex; flex-direction: column; align-items: center; justify-content: center; }
		.instagram-auth h3 { font-family: 'Open Sans', sans-serif; font-size: 30px; font-weight: 300; color: black; margin-bottom: 10px; }
		.instagram-auth p { font-family: 'Open Sans', sans-serif; font-size: 20px; font-weight: 300; color: #888888; text-align: center; }
		.instagram-auth img { width: 172px; height: auto; }
	</style>
	<?php
}
