...
class admin_api {
	...
	public function __construct() {
		...
		// project panel
		require get_template_directory() . '/admin/customize/projectpanel.php';
	...
	public function register_routes() {
		...
		// remove category
		register_rest_route($namespace, '/category/remove', array(
			'methods' => WP_REST_Server::CREATABLE,
			'permission_callback' => array($this, 'auth_user'),
			'callback' => array($this, 'remove_category'),
		));
	...
	public function init_editor($request) {
		...
		if(semplice_boolval($request['is_template']) === true && null !== $row && isset($row->content)) {
			// change ids
			$row->content = json_encode(semplice_generate_ram_ids($row->content, true, false), JSON_FORCE_OBJECT);
			// assign content
			$post_settings = false;
		} 
