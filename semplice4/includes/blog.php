...
class semplice_blog {
		...
		if($query->have_posts()) {
			...
			while($query->have_posts()) {
				...
				if(!$is_single) {
					...
					// get comments
					$comments = get_comments('post_id=' . get_the_ID() . '&orderby=comment_parent');
	...
	public function get_options() {
		...
		if(is_array($customize_blog)) {
			...
			// tags visibility
			if(isset($customize_blog['blog_visibility_tags']) && $customize_blog['blog_visibility_tags'] == 'visible') {
				$options['display_tags'] = 'visible';
			}
			// share type
	...
	public function date_comments_link($id, $options, $permalink) {
		...
		if(get_post_type($id) == 'post') {
			...
			if (comments_open($id)) {
				...
				$write_comments = '<a href="' . get_comments_link($id) .'"' . $options['head_meta_font'] . '>'. $comments.'</a>';
			} else {
				$write_comments =  __('Comments are off for this post.');
			}

			return '<p><a href="' . $permalink . '"' . $options['head_meta_font'] . '>' . get_the_date() . '</a><span' . $options['head_meta_font'] . '> &mdash; </span>' . $write_comments . '</p>';
		} else {
			return '';
		}
	}
	...
	public function customize() {
		...
		// feature image
		$featured_image = get_semplice_image('semplice_image_6.jpg');
		...
		$output = '
			<section class="post is-content post-1 post type-post status-publish format-standard hentry category-work' . $options['has_featured_full'] . '">
				<div class="container ' . $options['featured_preview'] . ' featured-image' . $options['featured_full'] . '">
						<div class="row">
							<div class="column" data-xl-width="' . $options['featured_width'] . '">
						   		<a href="a"><img src="' . $featured_image['url'] . '" alt="Featured Image"></a>
							</div>
						</div>
					</div>
				<div class="container">
					<div class="row">
						<div class="column post-column" data-xl-width="' . $options['width'] . '">
							<div class="post-heading">
								<p><a href="#"' . $options['head_meta_font'] . '>April 8, 2017</a><span' . $options['head_meta_font'] . '> &mdash; </span><a href="#"' . $options['head_meta_font'] . '>2 Comments</a></p>
